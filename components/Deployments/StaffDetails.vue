    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="mb-4">
            ...
            <OverlayPanel
                ref="searchOverlay"
                :appendTo="'body'"
                class="staff-search-overlay max-w-sm w-full transition-all ease-in-out"
            >
                <div class="flex flex-column gap-3 w-25rem">
                    ...
                    <div v-else-if="filteredStaff.length > 0">
                        <div
                            class="max-h-80 overflow-y-auto transition-all ease-in-out"
                            v-auto-animate
                        >
                            <div
                    ...
                    >
                        ...
                        >
                            ...
                            <Button
                                ...
                                @click="removeStaffFromSelection(invitedStaff)"
                            />
                        </div>
                    </div>
                </div>
...
// Initialize staff data from props
onMounted(() => {
    staff.value = props.staffList || [];
});

// Watch for changes in staffList prop and update local data
watch(
    () => props.staffList,
    (newValue) => {
        if (newValue) {
...
const filters = ref({
    ...
    isRegular: { value: null, matchMode: FilterMatchMode.EQUALS },
...
const showSearchOverlay = () => {
    if (searchOverlay.value && searchInput.value) {
        // Check if the overlay is not already visible
        if (!searchOverlay.value.visible) {
            // Use the search input element as the target
            // const target = searchInput.value.$el || searchInput.value;
            searchOverlay.value.show(searchInput.value);
        }
    }
};
